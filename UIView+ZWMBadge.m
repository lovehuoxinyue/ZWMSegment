...
- (void)gradientViewFrame:(CGRect)frame Horizontal:(BOOL)hori firstColor:(UIColor *)f_color lastColor:(UIColor *)l_color CornerRadius:(CGFloat)corner {
    CAGradientLayer* onlineVideoViewColorsLayer = [CAGradientLayer new];
    onlineVideoViewColorsLayer.frame = frame;
    onlineVideoViewColorsLayer.colors = @[(id)f_color.CGColor, (id)l_color.CGColor];
    if (hori) {
        onlineVideoViewColorsLayer.startPoint = CGPointMake(0, 0);
        onlineVideoViewColorsLayer.endPoint = CGPointMake(1, 0);
    }else {
        onlineVideoViewColorsLayer.startPoint = CGPointMake(0, 0);
        onlineVideoViewColorsLayer.endPoint = CGPointMake(0, 1);
    }
    onlineVideoViewColorsLayer.cornerRadius = corner;
    [self.layer addSublayer:onlineVideoViewColorsLayer];
}
...
- (void)addNumberBadge:(NSInteger)number badgeOffsetSize:(CGSize)size color:(UIColor *)color borderColor:(UIColor *)bColor {
    ...
    numberLabel.textColor = kWhiteColor;
